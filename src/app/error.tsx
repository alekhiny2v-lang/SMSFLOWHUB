...
export default function Error({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  ...
  return (
    <div className="min-h-screen bg-canvas text-fg grid place-items-center px-4">
      <div className="w-full max-w-md rounded-2xl border border-white/10 bg-surface/80 p-6 text-center shadow-2xl">
        ...
        {error?.message ? (
          <p className="mt-4 rounded-xl border border-white/5 bg-canvas/70 px-3 py-2 text-left font-mono text-[11px] break-words text-muted">
            {error.message}
          </p>
        ) : null}
        ...
        <p className="mt-5 text-[11px] text-muted">
          If it keeps failing, open <Link href="/api/health" className="text-brand-soft underline">/api/health</Link> to

  ...
  return (
    <AdminLayout>
      <PageHero
        ...
        icon={<span>📊</span>}
      >
        <span className="hidden sm:inline-flex items-center gap-2 rounded-xl bg-white/5 border border-white/10 px-3.5 py-2 text-[11px] font-bold text-fg-soft">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 lg:gap-5 mt-4 lg:mt-5">
        <div className="lg:col-span-2 rounded-2xl border border-white/10 bg-surface/90 shadow-xl p-5 lg:p-6">
          <div className="flex items-center justify-between gap-3 mb-5 flex-wrap">
            <h3 className="font-bold text-white">Activation breakdown</h3>
            <span className="text-[11px] text-muted font-semibold">{total} total</span>
          </div>
          <div className="grid grid-cols-3 gap-3 mb-5">
            {[
              { label: "Completed", value: act?.completed ?? 0, cls: "text-emerald-400", bar: "bg-emerald-500" },
              { label: "Pending", value: act?.pending ?? 0, cls: "text-info", bar: "bg-info" },
              { label: "Cancelled", value: act?.cancelled ?? 0, cls: "text-red-400", bar: "bg-red-500/70" },
            ].map((s) => (
              <div key={s.label} className="rounded-xl bg-canvas/50 border border-white/5 px-3.5 py-3">
          ...
          <p className="text-[11px] text-muted">Completion rate: <span className="text-fg-soft font-bold">{total > 0 ? Math.round(((act?.completed ?? 0) / total) * 100) : 0}%</span> of all activations ended with a received code.</p>
        </div>

        <div className="rounded-2xl border border-brand/25 bg-gradient-to-br from-brand/15 via-surface/60 to-surface/30 shadow-xl p-5 lg:p-6 relative overflow-hidden">
          <div className="absolute -right-12 -top-14 h-36 w-36 rounded-full bg-brand/25 blur-3xl" aria-hidden="true" />
          <p className="relative text-[10px] uppercase tracking-widest text-muted font-bold">Provider wallet</p>
          <p className="relative text-4xl font-bold text-white tabular-nums mt-2">{balance ? `$${balance.balance.toFixed(2)}` : "—"}</p>
          <p className="relative text-xs text-muted mt-2 leading-relaxed">
            Your SMSBOWER credit funds every number a client buys. Keep it topped up to avoid failed orders.
          </p>
          <div className="relative mt-4 rounded-xl bg-canvas/50 border border-white/5 px-3.5 py-3">
            <p className="text-[10px] uppercase tracking-widest text-muted font-bold">Activations served</p>
